...
\begin{defn}
We can demonstrate that the unitary operator corresponding to the phase shift in the iteration is $2\ket{0}\bra{0}-\hat{I}$. The combined effect of steps~\ref{enum:first-hadamard},\ref{enum:phase-shift} and~\ref{enum:second-hadamard} is then\footnote{Recall from~\ref{sec:hadamard} that $\hat{H}^{\otimes n} \ket{0} = \frac{1}{\sqrt{N}} \sum_{x=0}^{N-1}\ket{x}$.}:
\begin{equation*}
...
Step~\ref{enum:initial-state} it is simply the initial state. Step~\ref{enum:hadamard} consist of applying $H^{\otimes n}$ to the first $n$ qubits and $H X$ to the last qubit. Step~\ref{enum:iteration} is the Grover iteration defined in~\ref{eq:grover-iteration} applied $R$ times and step~\ref{enum:result} is the result.
