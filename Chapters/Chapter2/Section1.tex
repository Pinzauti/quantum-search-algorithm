...
\begin{enumerate}
  \item Apply the \emph{subroutine} $\hat{U}_w$ (we are going to discuss it in section~\ref{sec:subroutine}).
...
\begin{equation*}
    \hat{U}_s \equiv 2 \ket{\psi}\bra{\psi} - \hat{I}.
\end{equation*}
\end{defn}
\begin{defn}
We can define the Grover iteration as
\begin{equation}\label{def:grover-iteration}
G \equiv \hat{U}_s\hat{U}_w = (2 \ket{\psi}\bra{\psi} - \hat{I}) \hat{U}_w
\end{equation}
\end{defn}
\begin{figure}
...
\end{defn}
\begin{defn}
The subroutine $\hat{U}_w$ is a unitary operator defined by its action on the computational basis
\begin{equation*}
    \ket{x}\ket{q} \xrightarrow{\hat{U}_w} \ket{x} \ket{q \oplus f(x)}
\end{equation*}
\end{defn}
...
\begin{equation*}
    \hat{U}_w \ket{x} \biggl(\frac{\ket{0}-\ket{1}}{\sqrt{2}}\biggr) = (-1)^{f(x)} \ket{x} \biggl(\frac{\ket{0}-\ket{1}}{\sqrt{2}}\biggr).
\end{equation*}
We can see that the state of the oracle qubit is not changed; it turns out that this remains  $\frac{\ket{0}-\ket{1}}{\sqrt{2}}$ throughtout the iteration, and can therefore be omitted:
\begin{equation*}
    \hat{U}_w \ket{x} = (-1)^{f(x)} \ket{x}.
\end{equation*}
...
\begin{description}
   \item[Inputs] The inputs of the quantum computer are:
    \begin{enumerate}
  \item The subroutine $\hat{U}_w$ which perform the transformation $\hat{U}_w\ket{x}\ket{q} = \ket{x}\ket{q \oplus f(x)}$, where $f(x) = 0$ for all $0 \leq x \leq 2^n$ except $x_o$.
...
\end{enumerate}
  ...
  \item $\begin{aligned} \rightarrow \bigl[(2 \ket{\psi}\bra{\psi} - \hat{I})\hat{U}_w\bigr]^R \frac{1}{\sqrt{2^n}} \sum_{x=0}^{2^n-1}\ket{x} \biggl[\frac{\ket{0}-\ket{1}}{\sqrt{2}}\biggr] \approx \ket{x_0} \biggl[\frac{\ket{0}-\ket{1}}{\sqrt{2}}\biggr] \end{aligned}$ \label{enum:iteration}
